  ...
  */
  ...
  Pibble.prototype = {
    ...
    formatting: {
      ...
      // TODO: This needs a lot of tidying up after trying different methods
      blockquote: function() {
        ...
        // First pass - filter <br /> elements and see if all we have left is text nodes.
        // Covers FF with all top level nodes being text or <br />, and Chrome when 
        // undoing a list, which creates a <p> wrapping many text nodes and <br /> elements
        // todo: check for block level becaus of brs etc

        if (this.allTextNodes(this.filterOutEls(nodes, 'br'))) {
          var textNodes = this.filterOutEls(nodes, 'br');
          var brNodes = this.returnEls(nodes, 'br');

          var contentsMap = [];
          var contentsMapParents = [];

          // Text nodes with a parent node
          $(textNodes).filter(function() {
            return this.parentNode !== self.element[0];
          })
          .each(function(node) {
          ...
          // Text nodes with no parent node
          $(textNodes).filter(function() {
            return this.parentNode === self.element[0];
          })
          .wrap('<blockquote />');

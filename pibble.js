...
(function(window, $) {
  ...
  */
  ...
  // your choice: load this or load resource real in code
  // Add dinamic font-awesome 3.2.1
  var callbackFunc = function() { /*alert("loaded font");*/ };
  
  var head = document.getElementsByTagName("head")[0];
  var fileref = document.createElement("link");
  fileref.setAttribute("rel", "stylesheet");
  fileref.setAttribute("type", "text/css");
  fileref.setAttribute("href", "https://cdnjs.cloudflare.com/ajax/libs/font-awesome/3.2.1/css/font-awesome.min.css");
  ...
  function getToolbarHTML(formattingOptions, className) {
    ...
    for (var key in formattingOptions) {
      if (formattingOptions[key].enabled) {
	    var klass = formattingOptions[key].className;
        var el = $('<li />').data('format-option', formattingOptions[key].name);		  
		  // TODO: icons with fonts awesome by class - Text Editor Icons
		  // TODO: need adjust the itens marked after include tag <i>
		  el.append('<i class="'+klass+'"></i> '+klass.replace('icon-',''));  
  ...
  Pibble.prototype = {
    ...
    getUniques: function(arr) {
	  return arr.uniq(); // Method prototypeJS
    },
